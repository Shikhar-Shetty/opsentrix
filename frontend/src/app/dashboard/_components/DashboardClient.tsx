...
const TokenCell = ({ token }: { token?: string }) => {
  ...
  const copy = async () => { if (token) try { await navigator.clipboard.writeText(token) } catch {} }
  return (
    <div className="flex items-center gap-2">
      <code className="kbd kbd-sm">{masked}</code>
      <button className="btn btn-ghost btn-xs" onClick={() => setRevealed(r => !r)}>{revealed ? "Hide" : "View"}</button>
      <button className="btn btn-ghost btn-xs" onClick={copy}>Copy</button>
    </div>
  )
}
...
export default function DashboardClient({ initialAgents }: Props) {
  ...
  const generateToken = () => "tok_" + crypto.getRandomValues(new Uint8Array(12)).reduce((s,b)=>s+b.toString(16).padStart(2,"0"), "")
  const generateAgentId = () => "agt_" + crypto.getRandomValues(new Uint8Array(6)).reduce((s,b)=>s+b.toString(16).padStart(2,"0"), "")
  ...
  return (
    <div>
      <header className="mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button className="btn btn-outline btn-sm" onClick={() => setAgents([...agents])}>Refresh</button>
      ...
      <div className="card bg-base-200">
        <div className="card-body overflow-x-auto">
          <table className="table table-zebra">
            <thead>
              <tr>
                <th>Agent</th><th>Status</th><th>Last heartbeat</th>
                <th>CPU</th><th>Memory</th><th>Disk</th>
                <th>Proc</th><th>Token</th>
              </tr>
            </thead>
            <tbody>
              {agents.map(a => (
                <tr key={a.id}>
                  <td>
                    <div className="flex flex-col">
                      <Link href={`/agent/${a.id}`} className="link link-hover font-medium">{a.name}</Link>
                  ...
                  <td><TokenCell token={a.token} /></td>
                </tr>
              ))}
              {agents.length === 0 && <tr><td colSpan={8}><div className="flex justify-center py-8 opacity-70">No agents yet.</div></td></tr>}
            </tbody>
          </table>
        </div>
      </div>
      ...
      <div className={`modal ${creating ? "modal-open" : ""}`} role="dialog" aria-modal="true">
        <div className="modal-box focus:outline-none">
          ...
          <label className="form-control flex">
            <div className="label mr-2"><span className="label-text">Agent name:</span></div>
            <input
              type="text"
          ...
          <div className="label pt-3"><span className="label-text-alt opacity-70">You can't change this later.</span></div>
            ...
            <>
              <div className="alert alert-info mt-4">
                <div>
                  ...
                  <p className="text-sm opacity-90">Copy this token and use it to register your agent. It will also appear in the dashboard.</p>
                </div>
              </div>
              <div className="mt-3 flex items-center gap-2">
                ...
                <button
                  ...
                  onClick={async () => { try { await navigator.clipboard.writeText(creatingToken) } catch {} }}
                >
                  Copy
                </button>
              </div>
          ...
          <div className="modal-action">
            {!creatingToken ? (
              <>
                <button className="btn btn-ghost" onClick={closeCreateModal}>Cancel</button>
                <button className="btn btn-primary" onClick={onCreateAgent} disabled={!newName.trim()}>Create & Generate Token</button>
              </>
            ) : (
